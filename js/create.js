...
let work_words = ['Backspace','Tab','Delete','Enter','ArrowLeft','ArrowUp','ArrowDown','ArrowRight','ControlLeft','ControlRight','MetaLeft','AltLeft','AltRight'];
...
function spanInner(button, spanName) { // Заполнение кнопок
    ...
    console.log(key[0]);
    if(!(spanName[1] == 'ShiftLeft' || spanName[1] == 'ShiftRight')) {button.onclick = function() { clk(span); } }
...
function clk(key) {
    ...
    if(key[0].classList[0] == 'CapsLock') {
        if(key[0].parentNode.classList.contains('active')) { key[0].parentNode.classList.remove('active'); } 
        else { key[0].parentNode.classList.add('active'); }
        upCase('CapsLock');
    } else if(key[0].classList[0] == 'Backspace' || key[0].classList[0] == 'Delete') {
        key[0].parentNode.classList.add('active');
        if(key[0].classList[0] == 'Backspace') { textarea.value = textarea.value.slice(0,pos-1)+(textarea.value.slice(pos)); textarea.selectionStart = pos-1; textarea.selectionEnd = pos;} 
        else { textarea.value = textarea.value.slice(0,pos)+(textarea.value.slice(pos+1)); textarea.selectionStart = pos; textarea.selectionEnd = pos+1;}
